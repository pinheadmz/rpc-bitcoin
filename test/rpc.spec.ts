...
suite("RPCClient", () => {
  test(".batch()", async () => {
    ...
    const response = [
      {
        result:
          "000000000b11fd32393e0d259ab41e6995d94933c353de4a7dd7ad4bf5d6368c",
        error,
        id: 1,
      },
      {
  ...
  test(".rpc() (create a wallet)", async () => {
    const method = "createwallet";
    const params = { wallet_name: wallet };
    const response = { name: wallet };
    const data = await client.rpc(method, params);
    assert.deepStrictEqual(data, response);
  });
  ...
  suite("Blockchain", () => {
    ...
    test(".getblock()", async () => {
      const blockhash =
        "0000000044bef5d0b41387e3d155323db0ae112a5f8fddd8a313e90424dfc0fd";
      const verbosity: 2 = 2;
      const params = { blockhash, verbosity };
      const result = {
        hash: "0000000044bef5d0b41387e3d155323db0ae112a5f8fddd8a313e90424dfc0fd",
        confirmations: 35,
        height: 66,
        version: 536870912,
        versionHex: "20000000",
        merkleroot:
          "bd1c71d5604df4b0de01d20d63597883a593116ad4ffb7fb562787e1aa6f20e8",
        time: 1715002648,
        mediantime: 1715002505,
        nonce: 3819438276,
        bits: "1d00ffff",
        target:
        ...
        difficulty: 1,
        chainwork:
          "0000000000000000000000000000000000000000000000000000004300430043",
        nTx: 1,
        previousblockhash:
          "00000000ed73f132c9c4693d3cfed01da660b2ad472233a3abe05db2a37efda5",
        nextblockhash:
          "0000000055e0e6e406814eb5ac4b43aa22aa5b85e88812eb4b37a5c8ac7e1b7e",
        strippedsize: 222,
        size: 258,
        weight: 924,
        tx: [
          {
            ...
            hash: "79c476012110cb164aa91995b9349e1a964268abca598d03e9c51ec1f28eb436",
            version: 1,
            size: 177,
            vsize: 150,
            weight: 600,
            ...
            vout: [
              {
                ...
                n: 0,
                scriptPubKey: {
                  asm: "OP_DUP OP_HASH160 0a59837ccd4df25adc31cdad39be6a8d97557ed6 OP_EQUALVERIFY OP_CHECKSIG",
                  desc: "addr(mgTgHVFXFdMEJiMmLhGrxu75waDYjCjDvN)#59kuv0xn",
                  hex: "76a9140a59837ccd4df25adc31cdad39be6a8d97557ed688ac",
                  address: "mgTgHVFXFdMEJiMmLhGrxu75waDYjCjDvN",
                  type: "pubkeyhash",
                },
              },
              {
            ...
            hex: "",
          },
        ],
      };
    ...
    test(".getblockchaininfo()", async () => {
      const result = {
        ...
        difficulty: 1,
        time: 1715003483,
        mediantime: 1715003390,
        initialblockdownload: true,
        chainwork:
          "0000000000000000000000000000000000000000000000000000006500650065",
        pruned: false,
      };
      const data = await client.getblockchaininfo();
      assert(Object.entries(result).every(([key, val]) => data[key] === val));
    });
    ...
    test(".getblockfilter()", async () => {
      const blockhash =
        "00000000f607d20abf4bb15a698f3b0359b8a1afc79e6ed5e54c310271cde93e";
      const filtertype = "basic";
      const result = {
        filter: "01970cd0",
        header:
          "a6e4967ea4fb8402387f2ae0ca4d4532e184e7c0e719a298bd567cffbb9ce06b",
      };
    ...
    test(".getblockhash()", async () => {
      ...
      const params = { height };
      const result =
        "00000000d44c62992ba8d78a3efbc970fbdad10ab00087217bfba47359fa3033";
    ...
    test(".getblockheader()", async () => {
      ...
      const result =
        "00000020754f835e1f50a279c7c81a80526c4572789b0e6c25bf33d478be5ec60000000091fb3ddfc3b6a4f5c9e2ba2c2a5095a488725ba155ce29dd1383813b0f5bfba7aed93866ffff001daad5dab8";
